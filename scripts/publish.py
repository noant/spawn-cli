#!/usr/bin/env python3
"""Build the spawn-cli package with uv and publish wheels/sdists to PyPI.
...
Authentication (CLI overrides environment):
  SPAWN_CLI_PYPI_TOKEN  Environment variable (preferred for interactive use).
...
Usage:
  SPAWN_CLI_PYPI_TOKEN=... python scripts/publish.py
...
def main() -> None:
    ...
    if not pyproject.is_file():
        print(
            f"{_MSG_PREFIX} pyproject.toml not found next to repo root.",
            file=sys.stderr,
        )
        raise SystemExit(1)
